// app.draftitems.tsx
// — app.products.tsx를 기반으로 draft 전용 화면 생성
// status=DRAFT 필터를 고정하고, UI에서 Status & Published 제거
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  if (!productRes.ok || !metaRes.ok) throw new Response("GraphQL query failed", { status: 500 });
  ...
  const collectionOptions = metaJson.data.collections.edges.map((e: any) => ({ label: e.node.title, value: e.node.id }));
  const tagOptions = metaJson.data.productTags.edges.map((e: any) => ({ label: e.node, value: e.node }));

  return json<LoaderResponse>({ products, pageInfo: data.pageInfo, shop, collectionOptions, tagOptions });
};
...
export default function DraftItemsPage() {
  ...
  return (
    <Page title="Draft Products Page" fullWidth>
      <Layout>
        <Layout.Section>
          <Card>
            <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem", padding: "1rem" }}>
              <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                ...
                <Button onClick={() => {
                  setSearch(""); setCollection(""); setTag("");
                  setSearchParams({});
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: "0.5rem" }}>
                ...
                <Button size="slim" onClick={() => {
                  ...
                  params.set("reverse", (!currentReverse).toString());
                  window.location.search = params.toString();
                }}>{reverse ? "⬇️ Desc" : "⬆️ Asc"}</Button>
              </div>
            </div>
          </Card>
                    ...
                    <IndexTable.Row id={p.id} key={p.id} selected={resourceState.selectedResources.includes(p.id)} position={index}>
                      ...
                      <IndexTable.Cell>
                        <div style={{ wordWrap: "break-word", whiteSpace: "normal" }}>
                          <a href={`https://${shop}/admin/products/${p.id.split("/").pop()}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} style={{ color: "#1a73e8", textDecoration: "underline", cursor: "pointer", display: "inline-block" }}>{p.title}</a>
                        </div>
                      </IndexTable.Cell>
                      ...
                      <IndexTable.Cell><Text variant="bodySm" as="span" color="subdued"><div style={{ whiteSpace: "normal", wordWrap: "break-word" }}>{p.descriptionHtml?.replace(/<[^>]+>/g, "")}</div></Text></IndexTable.Cell>
                    </IndexTable.Row>
                  ))}
                </IndexTable>
